        ...
        <div className="min-h-screen bg-surface text-on-surface">
            ...
            <header className="fixed top-0 z-50 w-full bg-white/85 shadow-[0px_12px_32px_rgba(15,23,42,0.06)] backdrop-blur-md">
                <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                    <Link href={route('leagues.index')} className="flex items-center gap-3">
                    ...
                    <nav className="hidden items-center gap-2 md:flex">
                        {navItems.map(([key, label, icon, routeName]) => {
                            ...
                            return (
                                <Link
                                    key={key}
                                    href={routeName === 'profile.show' ? (user ? route('profile.show') : loginHref) : route(routeName)}
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        {user ? (
                            ...
                            <Link href={loginHref} className="rounded-full bg-primary px-4 py-2 text-sm font-bold text-on-primary shadow-sm hover:bg-primary/90">
                                Sign In
                            </Link>
                        )}
                    </div>
